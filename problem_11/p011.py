...
def max_product1():
    return max(grid[i][j]*grid[i][j+1]*grid[i][j+2]*grid[i][j+3] for i in range(19) for j in range(17))


def max_product2():
    return max(grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j] for i in range(17) for j in range(20))


def max_product3():
    return max(grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3] for i in range(17) for j in range(17))


def max_product4():
    return max(grid[i][j]*grid[i-1][j+1]*grid[i-2][j+2]*grid[i-3][j+3] for i in range(19, 3, -1) for j in range(17))


print("Solution ", max((max_product1(), max_product2(), max_product3(), max_product4())))
